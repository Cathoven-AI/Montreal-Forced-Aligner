...
"""
Modified Acoustic Model Training classes using a Polars-based database.

This version replaces SQLAlchemy session/engine calls with direct access to the
PolarsDB instance (attached in the worker), and includes a minimal ModelExporterMixin
to satisfy the import in this module.
"""
...
# Kalpy-related imports stay the same
from _kalpy.gmm import AccumAmDiagGmm
from _kalpy.matrix import DoubleVector
from kalpy.gmm.utils import read_gmm_model, write_gmm_model
from kalpy.utils import kalpy_logger

# Remove SQLAlchemy imports since we're not using them anymore.
# from sqlalchemy.orm import Session
# from sqlalchemy.engine import Engine

from montreal_forced_aligner import config

# Import base classes from our ABC module.
# (If ModelExporterMixin isn’t yet included there, we define a placeholder below.)
from montreal_forced_aligner.abc import MfaWorker, TrainerMixin, ModelExporterMixin
from montreal_forced_aligner.alignment import AlignMixin
from montreal_forced_aligner.alignment.multiprocessing import AccStatsArguments, AccStatsFunction
from montreal_forced_aligner.corpus.acoustic_corpus import AcousticCorpusPronunciationMixin
from montreal_forced_aligner.corpus.features import FeatureConfigMixin
from montreal_forced_aligner.data import PhoneType
# Instead of importing SQLAlchemy ORM models, we assume that equivalent tables exist
# in the Polars database (e.g., corpus_workflow, phone, utterance).
# from montreal_forced_aligner.db import CorpusWorkflow, Phone, Utterance
...
if TYPE_CHECKING:
    from montreal_forced_aligner.abc import MetaDict
    # Placeholder type for Job; adjust as needed.
    from typing import Any as Job
...
class AcousticModelTrainingMixin(
    ...
    """
    Base trainer class for training acoustic models and ivector extractors using a Polars-based DB.

    This class replaces SQLAlchemy database calls with fast, in-memory Polars DataFrame operations.
    It expects the worker (an instance of AcousticCorpusPronunciationMixin) to have a `polars_db`
    attribute with access to the exported tables.
    """
    architecture = "gmm-hmm"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.training_complete = False
        self.optional = optional
        self.realignment_iterations: List[int] = []  # To be set later
        self.final_gaussian_iteration: int = 0       # To be set later
        # Note: session/engine references are replaced by self.worker.polars_db calls

    @property
    def db_string(self) -> str:
        """A placeholder string for the Polars DB connection."""
        return "polars_based_db"

    def acc_stats_arguments(self) -> List[AccStatsArguments]:
        """
        Generate job arguments for AccStatsFunction using the PolarsDB instance.

        Instead of passing a SQLAlchemy session or connection string, we pass the PolarsDB.
        """
        arguments = []
        for j in self.jobs:
            arguments.append(
                AccStatsArguments(
                    j.id,
                    self.worker.polars_db,  # Pass the PolarsDB instance
                    self.working_log_directory.joinpath(f"acc.{self.iteration}.{j.id}.log"),
                    self.working_directory,
                    self.model_path,
                )
    ...
    @property
    def previous_aligner(self) -> AcousticCorpusPronunciationMixin:
        """Provide access to the previous aligner (the worker)."""
        return self.worker

    def utterances(self) -> pl.DataFrame:
        """
        Retrieve all utterances from the worker's Polars database.
        
        Returns
        -------
        pl.DataFrame
            The utterance table from the PolarsDB.
        """
        return self.worker.polars_db.get_table("utterance")

    @property
    def jobs(self) -> List[Job]:
        """Return the worker's job objects."""
        return self.worker.jobs

    @property
    def data_directory(self) -> str:
        """Return the current data directory based on the subset."""
        return self.worker.data_directory

    @property
    def corpus_output_directory(self) -> str:
        """Return the directory of the corpus output."""
        return self.worker.corpus_output_directory

    @property
    def num_current_utterances(self) -> int:
        """Return the number of utterances currently being used."""
        if self.subset:
            return self.subset
        return self.worker.num_utterances
    ...
    def workflow(self):
        """
        Retrieve the current workflow via the PolarsDB.

        Returns
        -------
        A row (as a tuple or equivalent) from the corpus_workflow table corresponding to this training's identifier,
        or None if not found.
        """
        wf_table = self.worker.polars_db.get_table("corpus_workflow")
        filtered = wf_table.filter(pl.col("name") == self.identifier)
        return filtered.row(0) if filtered.height > 0 else None

    def initialize_training(self) -> None:
        """Initialize training and compute necessary properties."""
        begin = time.time()
        logger.info(f"Initializing training for {self.identifier}...")
        if self.subset and self.subset >= self.worker.num_utterances:
            logger.warning(
                "Subset specified is larger than the dataset, using the full corpus for this training block."
            )
            self.subset = 0
            self.worker.current_subset = 0
        self.working_log_directory.mkdir(parents=True, exist_ok=True)
        ...
        self.current_gaussians = self.initial_gaussians
        logger.info("Initialization complete!")
        logger.debug(f"Initialization took {time.time() - begin:.3f} seconds")

    @abstractmethod
    def _trainer_initialization(self) -> None:
        """Descendant classes override this for their own training initialization."""
        pass

    def acc_stats(self) -> None:
        """
        Accumulate statistics and update acoustic models using kalpy.

        For Polars, we retrieve required accumulator tables from self.worker.polars_db.
        """
        log_path = self.working_log_directory.joinpath(f"acc.{self.iteration}.log")
        with kalpy_logger("kalpy.train", log_path) as train_logger:
            train_logger.debug(f"Model path: {self.model_path}")
            train_logger.debug(f"Next model path: {self.next_model_path}")
            train_logger.debug(f"Current gaussians: {self.current_gaussians}")
            train_logger.debug(f"Power: {self.power}")
            # Use Polars tables for accumulators (assuming they were populated elsewhere)
            transition_accs = self.worker.polars_db.get_table("transition_accs")
            gmm_accs = self.worker.polars_db.get_table("gmm_accs")
            objf_impr, count = self.transition_model.mle_update(transition_accs)
            train_logger.debug(
                f"Transition model update: {objf_impr / count} log-like improvement per frame over {count} frames."
            )
            objf_impr, count = self.acoustic_model.mle_update(
                gmm_accs, mixup=self.current_gaussians, power=self.power
            )
            train_logger.debug(
                f"GMM update: {objf_impr / count} objective function improvement per frame over {count} frames."
            )
            # Retrieve total likelihood (assumes a single row exists in the gmm_accs table)
            tot_like = gmm_accs.select(pl.col("TotLogLike")).to_numpy()[0][0]
            tot_t = gmm_accs.select(pl.col("TotCount")).to_numpy()[0][0]
            train_logger.debug(
                f"Average Likelihood per frame for iteration {self.iteration} = {tot_like / tot_t} over {tot_t} frames."
            )
            logger.debug(f"Log likelihood for iteration {self.iteration}: {tot_like / tot_t}")
            write_gmm_model(str(self.next_model_path), self.transition_model, self.acoustic_model)

    def align_iteration(self) -> None:
        """Perform alignment for one training iteration."""
        begin = time.time()
        self.align_utterances(training=True)
        logger.debug(
            f"Generating alignments for iteration {self.iteration} took {time.time() - begin:.3f} seconds"
        )

    @property
    def initialized(self) -> bool:
        """Check whether a model file exists indicating initialization is complete."""
        return (
            self.working_directory.joinpath("1.mdl").exists()
            or self.working_directory.joinpath("final.mdl").exists()
            or self.working_directory.joinpath("done").exists()
    ...
    def meta(self) -> MetaDict:
        """
        Construct metadata for the acoustic model training.
        This gathers information from the worker using Polars DB queries.
        """
        from datetime import datetime
        # Retrieve non-silence phones from the "phone" table
        phone_df = self.worker.polars_db.get_table("phone")
        non_silence_df = phone_df.filter(~pl.col("kaldi_label").str.startswith("#"))
        non_silence_phones = non_silence_df.get_column("kaldi_label").to_list()
        phone_mapping = {p: i for i, p in enumerate(non_silence_phones)}
        duration = 0.0  # Placeholder: compute total duration as needed
        utterance_df = self.worker.polars_db.get_table("utterance")
        utterance_count = utterance_df.height if utterance_df is not None else 0
        average_log_likelihood = 0.0  # Placeholder value; compute as needed
        dict_table = self.worker.polars_db.get_table("dictionary")
        default_dict = dict_table.get_column("id").to_list()[0] if dict_table.height > 0 else 0

        data = {
            "phones": sorted(list(set(non_silence_phones))),
            "phone_mapping": {k: v for k, v in phone_mapping.items() if not k.startswith("#")},
            "phone_groups": self.worker.phone_groups,
