...
"""
Polars-based Abstract Base Classes for Montreal Forced Aligner
================================================================

This module defines base classes for Kaldi functions, temporary directory handling,
and database functionality. Database operations are now implemented using an
in‑memory Polars-based database (see db_polars.py) to improve performance.
"""
...
class KaldiFunction(metaclass=abc.ABCMeta):
    ...
    def __init__(self, args: MfaArguments):
        self.args = args
        # In the original implementation we used either a string or a SQLAlchemy session.
        # With Polars we simply pass in a pre-loaded PolarsDB instance (if available) via args.
        self.db_string = None
        self._session = None
        self.polars_db = getattr(args, "polars_db", None)
        self.job_name = self.args.job_name
        self.log_path = self.args.log_path
        self.callback = None
    ...
    def session(self):
        """
        In a Polars-based database, sessions are not necessary.
        For backward compatibility, yield the pre-loaded PolarsDB instance if available.
        """
        yield self.polars_db

    def run(self):
        """
        Run the function, calling the subclass's _run method with error handling.
        """
        try:
            # Set threading based on whether a PolarsDB is provided.
            config.USE_THREADING = self.polars_db is not None
            self._run()
        except Exception:
            exc_type, exc_value, exc_traceback = sys.exc_info()
            error_text = "\n".join(
    ...
    def check_call(self, proc: subprocess.Popen):
        """
        Check whether a subprocess successfully completed.
        Raises KaldiProcessingError if the process did not complete successfully.
        """
        if proc.returncode is None:
            proc.wait()
        if proc.returncode != 0:
...
class TemporaryDirectoryMixin(metaclass=abc.ABCMeta):
    ...
    @property
    def model_output_directory(self) -> Path:
        """Temporary directory containing all model information."""
        return self.output_directory.joinpath("models")
    ...
    @property
    def language_model_output_directory(self) -> Path:
        """Temporary directory containing all language model information."""
        if self._language_model_output_directory:
            return self._language_model_output_directory
        return self.model_output_directory.joinpath("language_model")
    ...
    @property
    def acoustic_model_output_directory(self) -> Path:
        """Temporary directory containing all acoustic model information."""
        if self._acoustic_model_output_directory:
            return self._acoustic_model_output_directory
        return self.model_output_directory.joinpath("acoustic_model")
...
class DatabaseMixin(TemporaryDirectoryMixin, metaclass=abc.ABCMeta):
    """
    Abstract class for mixing in database functionality using a Polars-based DB.
    """

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._polars_db = None
        self.database_initialized = False
        self._current_workflow = None

    @property
    def polars_db(self):
        if self._polars_db is None:
            from montreal_forced_aligner.db_polars import PolarsDB
            # Use the output directory as the base directory for the Polars database.
            self._polars_db = PolarsDB()
        return self._polars_db

    def delete_database(self) -> None:
        """
        Delete or clear all tables from the Polars-based database.
        """
        for table_name in self.polars_db.tables:
            self.polars_db.tables[table_name] = pl.DataFrame()

    def initialize_database(self) -> None:
        """
        Initialize the Polars-based database with necessary tables.
        If config.CLEAN is True or the object is marked as dirty, then the existing working directory
        and the in-memory tables are cleared.
        """
        if self.database_initialized:
            return
        if config.CLEAN or getattr(self, "dirty", False):
            ...
            self.delete_database()
        os.makedirs(self.output_directory, exist_ok=True)
        # Ensure the "corpus_workflow" table exists.
        if (
            "corpus_workflow" not in self.polars_db.tables
            or self.polars_db.tables["corpus_workflow"].height == 0
        ):
            self.polars_db.tables["corpus_workflow"] = pl.DataFrame({
                "id": [],
                "name": [],
                "workflow_type": [],
                "working_directory": [],
    ...
    def get_next_primary_key(self, table_name: str) -> int:
        """
        Get the next primary key value for the specified table using Polars DB.
        """
        return self.polars_db.get_next_primary_key(table_name)

    def create_new_current_workflow(self, workflow_type: Any, name: str = None):
        """
        Create or update the current workflow using the Polars-based DB.
        """
        if name is None:
            name = workflow_type.name
        df = self.polars_db.get_table("corpus_workflow")
        if df is None or df.height == 0:
            ...
            new_workflow = {
                ...
                "working_directory": str(self.output_directory.joinpath(name)),
                "current": True,
            }
            new_df = pl.DataFrame(new_workflow, schema_overrides={
                "id": pl.Int64, "name": pl.Utf8, "workflow_type": pl.Utf8,
                "working_directory": pl.Utf8, "current": pl.Boolean
            })
            self.polars_db.update_table("corpus_workflow", new_df)
        else:
            df = df.with_columns(pl.lit(False).alias("current"))
            candidate = df.filter(pl.col("name") == name)
            if candidate.height == 0:
                new_pk = self.get_next_primary_key("corpus_workflow")
                new_workflow = {
                    "id": new_pk,
                    "name": name,
                    "workflow_type": str(workflow_type),
                ...
                df = df.vstack(pl.DataFrame(new_workflow))
            else:
                df = df.with_columns(
            ...
            self.polars_db.update_table("corpus_workflow", df)
        self._current_workflow = name

    def set_current_workflow(self, identifier):
        """
        Set the workflow with the given identifier as current.
        """
        df = self.polars_db.get_table("corpus_workflow")
        if df is None or df.height == 0:
            return
        df = df.with_columns(pl.lit(False).alias("current"))
        df = df.with_columns(
            pl.when(pl.col("name") == identifier)
            .then(True)
            .otherwise(pl.col("current"))
            .alias("current")
        )
    ...
    def current_workflow(self):
        """
        Retrieve the current workflow from the Polars-based database.
        """
        df = self.polars_db.get_table("corpus_workflow")
        if df is None or df.height == 0:
            return None
        current_df = df.filter(pl.col("current") == True)
        if current_df.height == 0:
            return None
        # Convert all rows to dictionaries and return the first one.
        return current_df.to_dicts()[0]
...
class TopLevelMfaWorker(MfaWorker, TemporaryDirectoryMixin, metaclass=abc.ABCMeta):
    """
    Abstract mixin for top-level workers in MFA.
    This class holds properties about the larger workflow run.
    """
    ...
    @classmethod
    def parse_args(cls, args: Optional[Dict[str, Any]], unknown_args: Optional[List[str]]) -> MetaDict:
        from montreal_forced_aligner.data import Language
        ...
        for name, param_type in param_types.items():
            if (name.endswith("_directory") and name != "audio_directory") or (
                ...
                continue
            if args is not None and name in args and args[name] is not None:
                if param_type.__name__ == "Language":
                    params[name] = param_type[args[name]]
                else:
                    params[name] = param_type(args[name])
            elif name in unknown_dict:
    ...
    def cleanup(self) -> None:
        try:
            if hasattr(self, "cleanup_connections"):
                pass  # Not needed in the Polars version.
            if self.dirty:
                logger.error("There was an error in the run, please see the log.")
            else:
                logger.info(f"Done! Everything took {time.time() - self.start_time:.3f} seconds")
